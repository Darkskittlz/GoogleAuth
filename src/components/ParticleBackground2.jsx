...
const baseParticleConfig = {
  ...
  interactivity: {
    ...
    modes: {
      ...
      repulse: {
        ...
        divs: { distance: 200, duration: 0.4, factor: 100, speed: 1, maxSpeed: 50, selectors: [] },
      },
  ...
  particles: {
    ...
    opacity: {
      value: { min: 0.1, max: 1 },
      animation: { count: 0, enable: true, speed: 1, decay: 0, delay: 0, sync: false, mode: "auto", startValue: "random", destroy: "none" }
    },
    ...
    destroy: {
      bounds: {}, mode: "none", split: { count: 1, factor: { value: 3 }, rate: { value: { min: 4, max: 9 } }, sizeOffset: true, particles: {} }
    },
    ...
    links: {
      blink: false, color: { value: "#fff" }, consent: false, distance: 100, enable: false, frequency: 1, opacity: 1, shadow: { blur: 5, color: { value: "#000" }, enable: false },
...
const ParticleBackground2 = () => {
  ...
  useEffect(() => {
    ...
    // Ensure the whole tsParticles is loaded
    async function loadParticles() {
      await loadFull(tsParticles);  // Load full tsParticles configuration


      const particleOptions = {
        ...baseParticleConfig,
        background: {
          ...baseParticleConfig.background,
          image: theme === 'light' ? 'url(/logo_rounded.png)' : 'url(/men_logo.png)',
          size: theme === 'light' ? '60%' : '30%',
          color: { value: theme === 'light' ? '#fa76ea' : '#232741' },
        },
        particles: {
          ...baseParticleConfig.particles,
          shape: { close: true, fill: true, options: {}, type: theme === 'light' ? 'star' : 'circle' }  // Adjust shape based on theme
        }
      };


      await tsParticles.load("tsparticles", particleOptions);
      // console.log('Styles:', theme === 'light' ? lightThemeStyle : darkThemeStyle);
    }
